...
public class MultiClient2 extends JFrame implements ActionListener
{
	...
	public MultiClient2(){										//it runs the constructor and enables all gui properties														
	...
	public void actionPerformed(ActionEvent ae) {					//method gets called on click of send button
		if (ae.getSource().equals(sendText)) {						//It checks which button to be called
			...
			try {										
				...
				Files.write(Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt"), "Vote Commit\n".getBytes(), StandardOpenOption.APPEND);	//writes in file
			...
			try {										
				...
				Files.write(Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt"), "Vote Abort\n".getBytes(), StandardOpenOption.APPEND);
			} catch (Exception e) {									//Handles exception if any occurred
				e.printStackTrace();
			}
		}
	...
	{	
		new MultiClient2();//Object is called on 
		Path display = Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2", "Client2.txt");			//reads file upon client starting
		try {
		...
		File yourFile = new File("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt");		//path to create file
		...
		while(true) {
			...
			if(msg.contains("Sending vote request")) {
				...
				Files.write(Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt"), store.getBytes(), StandardOpenOption.APPEND);		//writes in to local file
			}
				...
				timer.scheduleAtFixedRate(new TimerTask() {			//schedules  task for repeated fixed rate execution
					...
					public void run() {									// method is executed to start timer 
						...
						if(msg.contains("Coordinator Terminated")||i<0) {		//executes when coordinator is terminated or timer is finished
							try {
								if(i<0) {
									Files.write(Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt"), "abort\n".getBytes(), StandardOpenOption.APPEND);	//writes to file if timer is over
								}
							...
							String date=java.time.format.DateTimeFormatter.RFC_1123_DATE_TIME.format(ZonedDateTime.now(ZoneOffset.UTC)).toString();		
							 ...
							 String drhttp="\n"+post+"\n"+"Date:"+date+"\n"+ 
								...
								Files.write(Paths.get("C:\\Users\\aksha\\eclipse-workspace\\DLab2\\Client2.txt"), "Decision_Request\n".getBytes(), StandardOpenOption.APPEND);	//writes decision to local file
